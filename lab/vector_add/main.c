...
int main(int argc, char *argv[])
{
    ...
    clEnqueueWriteBuffer(queue, device_a, CL_TRUE, 0, host_a.shape[0]*host_a.shape[1]*sizeof(float), host_a.data, 0, NULL, NULL);
    clEnqueueWriteBuffer(queue, device_b, CL_TRUE, 0, host_b.shape[0]*host_b.shape[1]*sizeof(float), host_b.data, 0, NULL, NULL);

    // Set the arguments to our compute kernel
    unsigned int size_c = host_c.shape[0] * host_c.shape[1];
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(unsigned int), &size_c);
    CHECK_ERR(err, "clSetKernelArg 3");

    //@@ Initialize the global size and local size here
    global_item_size = size_c;
    local_item_size = 1;
