    ...
    const unsigned int numCRows, const unsigned int numCColumns)
{
  ...
  for (unsigned int tile = 0; tile < num_tiles; tile++)
  {
    ...
    {
      unsigned int tile_row = (tile * TILE_SIZE + i);
      unsigned int tile_col = (tile * TILE_SIZE + i);
      if ((row < numARows) && (tile_col < numAColumns))
      {
        A_pr[row_pr][i] = A[row * numAColumns + tile_col];
      }
      else
      {
        A_pr[row_pr][i] = 0;
      ...
      {
        B_pr[i][col_pr] = B[tile_row * numBColumns + col];
      }
      else
      {
        B_pr[i][col_pr] = 0;
